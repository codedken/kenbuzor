...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText("cbsmoothoj@gmail.com");
    setCopied(true);
  };
  return (
    <div
    ...
    >
      <div className={`${id === 6 && "flex justify-center"} h-full`}>
        ...
        >
          ...
          {id === 2 && <GlobeDemo />}
          {id === 3 && (
            <div className="absolute -right-3 flex w-fit gap-1 sm:gap-2 lg:-right-2">
              <div className="flex flex-col gap-1 sm:gap-2">
                {["React.js", "Next.js", "Nodejs"].map((item) => (
                  <span
                    key={item}
                    className="rounded-lg bg-[#10132E] px-3 py-2 text-center text-xs text-white-100 opacity-50 lg:text-base lg:opacity-100"
                  >
                    {item}
                  </span>
                ))}
              ...
              <div className="flex flex-col gap-1 sm:gap-2">
                <span className="rounded-lg bg-[#10132E] px-3 py-4 text-center" />
                {["flutter", "PostgreSql", "MongoDb"].map((item) => (
                  <span
                    key={item}
                    className="rounded-lg bg-[#10132E] px-3 py-2 text-center text-xs text-white-100 opacity-50 lg:text-base lg:opacity-100"
                  >
